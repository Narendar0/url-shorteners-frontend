...
function Dashboard() {
    ...
    useEffect(async () => {
        try {
            let dashboard = await axios.get("https://url-shorteners-backend.herokuapp.com/dashboard", {
                headers: {
    ...
    let fetchUrls = async () => {
        try {
            let allUrls = await axios.get("https://url-shorteners-backend.herokuapp.com/getUrls")
    ...
    let handleDelete = async (id) => {
        try {
            ...
            if (result) {
                await axios.delete(`https://url-shorteners-backend.herokuapp.com/url/${id}`)
                fetchUrls();
            }
        } catch (error) {
            console.log(error)
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            try {
                axios.post("https://url-shorteners-backend.herokuapp.com/create-url", values)
            } catch (error) {
                console.log(error)
            }
            fetchUrls();
